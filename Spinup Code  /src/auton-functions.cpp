...
void autonShoot(int num, float interval, float lowerbound, float upperbound, float lowerbound2, float upperbound2){
  clearShootCount();
  this_thread::sleep_for(100);
  while (getShootCount() < num){
    if (getShootCount() < num-1) triggerStatus(true, interval, lowerbound, upperbound);
    else{
      setFlywheelSpeed(upperbound2);
      triggerStatus(true, interval, lowerbound2, upperbound2);
    }
    // cout << getFlywheelSpeed() << endl;
    this_thread::sleep_for(5);
  }
